...
create table sprints (is_active number(1,0) check (is_active in (0,1)), created_at timestamp(6) not null, end_date timestamp(6) not null, id number(19,0) generated by default as identity, project_id number(19,0) not null, start_date timestamp(6) not null, updated_at timestamp(6), sprint_description TEXT, sprint_name varchar2(255 char) not null, status varchar2(255 char) check (status in ('ACTIVE','COMPLETED','PLANNING','CANCELLED')), primary key (id));
