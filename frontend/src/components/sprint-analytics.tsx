...
const generateBurndownData = (sprint: SprintData) => {
  ...
  let lastActual =
    ...
    (actualBurndown.length > 0 ? actualBurndown[actualBurndown.length - 1].actual : 0);
...
const generateVelocityData = (sprint: SprintData) => {
  if (!sprint) {
    return { statusCounts: { todo: 0, inProgress: 0, done: 0, blocked: 0 }, statusPoints: { todo: 0, inProgress: 0, done: 0, blocked: 0 } };
  }

  const statusCounts = {
    todo: sprint.tasks.filter((t) => t.status === TaskStatus.TODO && !t.blocked).length,
    inProgress: sprint.tasks.filter((t) => t.status === TaskStatus.IN_PROGRESS && !t.blocked).length,
    done: sprint.tasks.filter((t) => t.status === TaskStatus.DONE && t.completedAt).length,
    blocked: sprint.tasks.filter((t) => t.blocked && t.status !== TaskStatus.DONE).length,
  };
...
export function SprintAnalyticsDashboard() {
  ...
  const [sprintAnalyticsData, setSprintAnalyticsData] = useState<SprintForAnalyticsModel|null>(null);
  const [loading, setLoading] = useState(true);
  const [selectedSprint, setSelectedSprint] = useState<string>("");
  const userId = Number(session?.user?.id);
  const teamId = Number(session?.user?.teamId);

  const [sprintDataFormatted, setSprintDataFormatted] = useState<SprintData | null>(null);

  const burndownData = React.useMemo(() =>
          generateBurndownData(sprintDataFormatted || { id: "", name: "", startDate: "", endDate: "", tasks: [], goal: "" }),
      [sprintDataFormatted]
  );

  const velocityData = React.useMemo(() =>
          generateVelocityData(sprintDataFormatted || { id: "", name: "", startDate: "", endDate: "", tasks: [], goal: "" }),
      [sprintDataFormatted]
  );
  ...
  const tasksByAssignee = React.useMemo(() => {
    ...
    const assignees = [
      ...new Set(
          sprintDataFormatted.tasks.map((t) => t.assignee).filter(Boolean) as string[],
      ),
    ];
    return assignees.map((assignee) => ({
      name: assignee,
      value: sprintDataFormatted.tasks.filter((t) => t.assignee === assignee).length,
    }));
  }, [sprintDataFormatted]);
        ...
        .then((sprintAnalytics) => {
          ...
          if (sprintAnalytics) {
            ...
            const formattedData: SprintData = {
              ...
              goal: sprintAnalytics.sprintDescription || "No description provided",
              tasks: sprintAnalytics.tasks.map(task => ({
                ...
                completedAt: task.completedAt ? task.completedAt.toISOString().split('T')[0] : undefined
              }))
            };
  ...
  if (loading || !sprintAnalyticsData || !kpis || !teamKpis || !sprintDataFormatted) {
    return (
  ...
  return (
    <div className="space-y-4 mx-6 my-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold">{sprintDataFormatted.name} Analytics</h2>
        ...
        <div className="flex items-center gap-2">
          <Select value={selectedSprint} onValueChange={setSelectedSprint}>
            ...
            <SelectContent>
              <SelectItem value={sprintDataFormatted.id}>{sprintDataFormatted.name}</SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{((completedPoints / totalPoints) * 100).toFixed(1)}%</div>
      ...
      <Tabs defaultValue="burndown">
              ...
              <div className="flex justify-between items-center mt-4 text-sm">
                ...
                <div className="flex items-center">
                  {(() => {
                    ...
                    const isBehindSchedule = !isSprintComplete && (completedPoints / totalPoints) < ((14 - daysRemaining) / 14);
                    ...
                    data={[
                      {
                        ...
                        inProgress: sprintDataFormatted.tasks.filter(
                          (t) => t.priority === "HIGH" && t.status === TaskStatus.IN_PROGRESS,
                        ).length,
                        done: sprintDataFormatted.tasks.filter(
                          (t) => t.priority === "HIGH" && t.status === TaskStatus.DONE,
                        ).length,
                      ...
                      },
                      {
                        priority: "Medium",
                        todo: sprintDataFormatted.tasks.filter(
                          (t) => t.priority === "MEDIUM" && t.status === TaskStatus.TODO,
                        ).length,
                        inProgress: sprintDataFormatted.tasks.filter(
                          (t) =>
                            t.priority === "MEDIUM" &&
                          ...
                          (t) => t.priority === "MEDIUM" && t.status === TaskStatus.DONE,
                        ).length,
                        blocked: sprintDataFormatted.tasks.filter(
                          (t) =>
                            t.priority === "MEDIUM" && t.blocked,
                        ).length,
                      },
                      {
                        priority: "Low",
                        todo: sprintDataFormatted.tasks.filter(
                          (t) => t.priority === "LOW" && t.status === TaskStatus.TODO,
                        ).length,
                        inProgress: sprintDataFormatted.tasks.filter(
                          (t) =>
                            t.priority === "LOW" && t.status === TaskStatus.IN_PROGRESS,
                        ).length,
                        done: sprintDataFormatted.tasks.filter(
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {sprintDataFormatted.tasks.filter((t) => t.blocked).length > 0 ? (
        ...
        <TabsContent value="personal" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Personal Sprint Velocity</CardTitle>
                <CardDescription>Story points completed by you</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const userTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === session?.user?.name)
                  const userCompletedPoints = userTasks
                    .filter((t) => t.status === TaskStatus.DONE && t.completedAt)
                    .reduce((sum, t) => sum + t.storyPoints, 0)
                  const userTotalPoints = userTasks.reduce((sum, t) => sum + t.storyPoints, 0)

                  return (
                    <>
                      <div className="text-2xl font-bold">{userCompletedPoints} points</div>
                      <Progress value={(userCompletedPoints / Math.max(1, userTotalPoints)) * 100} className="h-2 mt-2" />
                      <p className="text-xs text-muted-foreground mt-2">
                        {Math.round((userCompletedPoints / Math.max(1, userTotalPoints)) * 100)}% of your assigned points completed
                      </p>
                    </>
                  )
                })()}
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Personal Completion Rate</CardTitle>
                <CardDescription>Percentage of your tasks completed</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const userTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === session?.user?.name)
                  const userCompletedTasks = userTasks.filter((t) => t.status === TaskStatus.DONE && t.completedAt).length
                  const completionRate = (userCompletedTasks / Math.max(1, userTasks.length)) * 100
                  ...
                  return (
                    <>
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        {userCompletedTasks} of {userTasks.length} tasks completed
                      </p>
                    </>
                  )
                })()}
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Story points per hour (estimated)</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              ...
              <CardContent>
                {(() => {
                  const userTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === session?.user?.name)
                  const completedTasks = userTasks.filter((t) => t.status === TaskStatus.DONE && t.completedAt)
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Bugs vs Features Ratio</CardTitle>
                <CardDescription>Balance of bug fixes to features</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  ...
                  return (
                    <>
                      <div className="text-2xl font-bold">{kpis.bugsVsFeatures.toFixed(2)}</div>
                      <div className="flex items-center justify-between text-xs mt-2">
                        <div className="flex items-center">
                          <span className="h-2 w-2 rounded-full bg-chart-5 mr-1"></span>
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Your workload vs team average</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  ...
                  const assignees = [...new Set(sprintDataFormatted.tasks.map((t) => t.assignee).filter(Boolean))]
                  const avgTeamPoints = totalPoints / Math.max(1, assignees.length)
                  const workloadRatio = userStoryPoints / Math.max(1, avgTeamPoints)

                  return (
                    <>
                      <div className="text-2xl font-bold">{workloadRatio.toFixed(2)}x</div>
                      <Progress
                        value={Math.min(workloadRatio * 50, 100)}
                        className={`h-2 mt-2 ${workloadRatio > 1.2 ? "bg-chart-5" : ""}`}
                      />
                      <p className="text-xs text-muted-foreground mt-2">
                        {userStoryPoints} points vs {avgTeamPoints.toFixed(1)} team average
                      </p>
                    </>
                  )
                })()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Personal risk factors for this sprint</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const userTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === session?.user?.name)

                  const blockedTasks = userTasks.filter((t) => t.blocked).length
                  ...
                  const userCompletedPoints = userTasks
                    .filter((t) => t.status === TaskStatus.DONE && t.completedAt)
                    .reduce((sum, t) => sum + t.storyPoints, 0)
                  const userRemainingPoints = userTotalPoints - userCompletedPoints
                  const userDaysElapsed = 14 - daysRemaining
                  const userAvgVelocity = userCompletedPoints / Math.max(1, userDaysElapsed)
                  const scopeAtRisk = userRemainingPoints > userAvgVelocity * daysRemaining
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col p-3 border rounded-md">
                          ...
                          <div className="text-2xl font-bold mt-1">{blockedTasks}</div>
                          <Badge
                            variant="outline"
                        ...
                        <div className="flex flex-col p-3 border rounded-md">
                          <div className="text-sm font-medium">High Priority Pending</div>
                          <div className="text-2xl font-bold mt-1">{highPriorityPending}</div>
                          <Badge
                            variant="outline"
                            className={`mt-2 w-fit ${highPriorityPending > 1 ? "border-chart-5 text-chart-5" : "border-chart-2 text-chart-2"}`}
                          >
                            {highPriorityPending > 1 ? "Attention Required" : "On Track"}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col p-3 border rounded-md">
                          ...
                          <div className="text-2xl font-bold mt-1">{userRemainingPoints} points left</div>
                          <Badge
                            variant="outline"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Additional personal performance metrics</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const userTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === session?.user?.name)
                  ...
                  const userCompletedPoints = userTasks
                    .filter((t) => t.status === TaskStatus.DONE && t.completedAt)
                    .reduce((sum, t) => sum + t.storyPoints, 0)
                  const percentTimeElapsed = Math.round(((14 - daysRemaining) / 14) * 100)
                  const percentWorkCompleted = Math.round((userCompletedPoints / Math.max(1, userTotalPoints)) * 100)
                  const featurePoints = userTasks
                    .filter((t) => t.type === "FEATURE" && t.status === TaskStatus.DONE && t.completedAt)
                    .reduce((sum, t) => sum + t.storyPoints, 0)
                  const focusScore = (featurePoints / Math.max(1, userCompletedPoints)) * 100
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col p-3 border rounded-md">
                        <div className="text-sm font-medium">Progress Forecast</div>
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-sm">{Math.round(percentTimeElapsed)}% time elapsed</span>
                          <span className="text-sm">{Math.round(percentWorkCompleted)}% work completed</span>
                        </div>
                        <Progress value={percentWorkCompleted} className="h-2 mt-2" />
                        <Badge
                          variant="outline"
                          className={`mt-2 w-fit ${percentWorkCompleted <= percentTimeElapsed ? "border-chart-5 text-chart-5" : "border-chart-2 text-chart-2"}`}
                        >
                          {percentWorkCompleted <= percentTimeElapsed ? "Behind Schedule" : "Ahead of Schedule"}
                        </Badge>
                      </div>

                      <div className="flex flex-col p-3 border rounded-md">
                        <div className="text-sm font-medium">Focus Score</div>
                        <div className="text-2xl font-bold mt-1">{Math.round(focusScore)}%</div>
                        <div className="text-xs text-muted-foreground mt-1">
                          {Math.round(focusScore)}% of your completed work is feature development
                        </div>
                      </div>
                    </div>
                  )
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Breakdown of your assigned tasks</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[300px]">
                {(() => {
                  const taskTypeBreakdown = kpis.taskTypeBreakdown || {}
                  const taskDistribution = Object.entries(taskTypeBreakdown).map(([name, value]) => ({
                    name,
                    value,
                  }))
        ...
        <TabsContent value="team" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Team Sprint Velocity</CardTitle>
                <CardDescription>Total story points completed by team</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{completedPoints} points</div>
                <Progress value={(completedPoints / totalPoints) * 100} className="h-2 mt-2" />
                <p className="text-xs text-muted-foreground mt-2">
                  {Math.round((completedPoints / totalPoints) * 100)}% of total points completed
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Percentage of all sprint tasks completed</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Average Efficiency Score</CardTitle>
                <CardDescription>Team&#39;s average story points per hour</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  return (
                    <>
                      <div className="text-2xl font-bold">{teamKpis.averageEfficiency.toFixed(2)}</div>
                      <div className="flex items-center mt-2">
                        <Clock className="h-4 w-4 text-muted-foreground mr-1" />
                        <span className="text-xs text-muted-foreground">Based on ~{teamKpis.totalTimeLogged} team hours</span>
                      </div>
                    </>
                  )
                })()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              ...
              <CardContent>
                {(() => {
                  const completedTasks = sprintDataFormatted.tasks.filter((t) => t.status === TaskStatus.DONE && t.completedAt)
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Team Bug:Feature Ratio</CardTitle>
                <CardDescription>Balance of bug fixes to features</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const bugTasks = sprintDataFormatted.tasks.filter((t) => t.type === "BUG").length
                  const featureTasks = sprintDataFormatted.tasks.filter((t) => t.type === "FEATURE").length
                  const ratio = featureTasks > 0 ? bugTasks / featureTasks : bugTasks
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Workload Balance Score</CardTitle>
                <CardDescription>Distribution of work across team</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
                  const assignees = [...new Set(sprintDataFormatted.tasks.map((t) => t.assignee).filter(Boolean))]
                  ...
                  const variance =
                    assigneePoints.reduce((sum, points) => sum + Math.pow(points - avgPoints, 2), 0) / assignees.length
                  ...
                  return (
                    <>
                      <div className="text-2xl font-bold">{balanceScore.toFixed(2)}</div>
                      <Progress value={balanceScore * 100} className="h-2 mt-2" />
                      <p className="text-xs text-muted-foreground mt-2">
                        {balanceScore > 0.8
                          ? "Well balanced"
                          : balanceScore > 0.5
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Team-wide risk factors for this sprint</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col p-3 border rounded-md">
                      <div className="text-sm font-medium">Blocked Tasks (Team)</div>
                      <div className="text-2xl font-bold mt-1">{velocityData.statusCounts.blocked}</div>
                      <Badge
                        variant="outline"
                        className={`mt-2 w-fit ${velocityData.statusCounts.blocked > 0 ? "border-chart-5 text-chart-5" : "border-chart-2 text-chart-2"}`}
                      >
                        {velocityData.statusCounts.blocked > 0 ? "Action Needed" : "All Clear"}
                      </Badge>
                    </div>
                    ...
                    <div className="flex flex-col p-3 border rounded-md">
                      <div className="text-sm font-medium">High Priority Pending</div>
                      <div className="text-2xl font-bold mt-1">
                        {sprintDataFormatted.tasks.filter((t) => t.priority === "HIGH" && t.status !== TaskStatus.DONE).length}
                      </div>
                      <Badge
                        variant="outline"
                      ...
                      >
                        {sprintDataFormatted.tasks.filter((t) => t.priority === "HIGH" && t.status !== TaskStatus.DONE).length > 2
                          ? "Attention Required"
                          : "On Track"}
                      </Badge>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col p-3 border rounded-md">
                      <div className="text-sm font-medium">Overdue Tasks</div>
                      <div className="text-2xl font-bold mt-1">
                        {sprintDataFormatted.tasks.filter((t) => new Date(t.dueDate) < new Date() && t.status !== TaskStatus.DONE).length}
                      </div>
                      <Badge
                        variant="outline"
                      ...
                      >
                        {sprintDataFormatted.tasks.filter((t) => new Date(t.dueDate) < new Date() && t.status !== TaskStatus.DONE)
                          .length > 0
                          ? "Past Due"
                          : "On Schedule"}
                      </Badge>
                    ...
                    <div className="flex flex-col p-3 border rounded-md">
                      <div className="text-sm font-medium">Sprint Scope</div>
                      <div className="text-2xl font-bold mt-1">{totalPoints - completedPoints} points left</div>
                      <Badge
                        variant="outline"
                      ...
                      >
                        {totalPoints - completedPoints > daysRemaining * (completedPoints / (14 - daysRemaining))
                          ? "At Risk"
                          : "Achievable"}
                      </Badge>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Additional team performance insights</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="flex flex-col p-3 border rounded-md">
                    <div className="text-sm font-medium">Time Logged This Sprint (Total)</div>
                    <div className="text-2xl font-bold mt-1">{teamKpis.totalTimeLogged} hours</div>
                    <div className="text-xs text-muted-foreground mt-1">
                  ...
                  <div className="flex flex-col p-3 border rounded-md">
                    <div className="text-sm font-medium">Progress Forecast (Team)</div>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-sm">{Math.round(((14 - daysRemaining) / 14) * 100)}% time elapsed</span>
                      <span className="text-sm">
                        {Math.round((completedPoints / totalPoints) * 100)}% work completed
                      </span>
                    </div>
                    <Progress value={(completedPoints / totalPoints) * 100} className="h-2 mt-2" />
                    <Badge
                    ...
                    >
                      {(completedPoints / totalPoints) * 100 >= ((14 - daysRemaining) / 14) * 100
                        ? "Ahead of Schedule"
                        : "Behind Schedule"}
                    </Badge>
                  </div>
                  ...
                  <div className="flex flex-col p-3 border rounded-md">
                    <div className="text-sm font-medium">Focus Score (Team)</div>
                    {(() => {
                      const featurePoints = sprintDataFormatted.tasks
                        .filter((t) => t.type === "FEATURE" && t.status === TaskStatus.DONE && t.completedAt)
                        .reduce((sum, t) => sum + t.storyPoints, 0)
                      const focusScore = (featurePoints / Math.max(1, completedPoints)) * 100

                      return (
                        <>
                          <div className="text-2xl font-bold mt-1">{Math.round(focusScore)}%</div>
                          <div className="text-xs text-muted-foreground mt-1">
                            {Math.round(focusScore)}% of completed work is feature development
                          </div>
                        </>
                      )
                    })()}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Team members with highest performance metrics</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {(() => {
                  const assignees = [...new Set(sprintDataFormatted.tasks.map((t) => t.assignee).filter(Boolean) as string[])]

                  const teamMetrics = assignees.map((assignee) => {
                    const assigneeTasks = sprintDataFormatted.tasks.filter((t) => t.assignee === assignee)
                    const completedTasks = assigneeTasks.filter((t) => t.status === TaskStatus.DONE && t.completedAt)
                    const completedPoints = completedTasks.reduce((sum, t) => sum + t.storyPoints, 0)
                    const featureTasks = completedTasks.filter((t) => t.type === "FEATURE")
                    const featurePoints = featureTasks.reduce((sum, t) => sum + t.storyPoints, 0)
                    ...
                    return {
                      ...
                      completedPoints,
                      featurePoints,
                      efficiency: completedPoints / (assigneeTasks.length || 1),
                    }
                  })

                  const sortedByPoints = [...teamMetrics].sort((a, b) => b.completedPoints - a.completedPoints)
                  const topContributor = sortedByPoints[0]

                  const sortedByEfficiency = [...teamMetrics].sort((a, b) => b.efficiency - a.efficiency)
                  const mostEfficient = sortedByEfficiency[0]

                  const sortedByFeatures = [...teamMetrics].sort((a, b) => b.featurePoints - a.featurePoints)
                  const topFeatureContributor = sortedByFeatures[0]

                  return (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="flex flex-col p-3 border rounded-md">
                        <div className="text-sm font-medium">Most Productive</div>
                        <div className="text-xl font-bold mt-1">{topContributor?.name || "N/A"}</div>
                        <div className="text-sm mt-1">{topContributor?.completedPoints || 0} points completed</div>
                        <div className="text-xs text-muted-foreground mt-1">
                          {topContributor?.completedTasks || 0} tasks completed
                        </div>
                      </div>

                      <div className="flex flex-col p-3 border rounded-md">
                        <div className="text-sm font-medium">Most Efficient</div>
                        <div className="text-xl font-bold mt-1">{mostEfficient?.name || "N/A"}</div>
                        <div className="text-sm mt-1">{mostEfficient?.efficiency.toFixed(1) || "0.0"} points per task</div>
                        <div className="text-xs text-muted-foreground mt-1">
                      ...
                      <div className="flex flex-col p-3 border rounded-md">
                        <div className="text-sm font-medium">Feature Champion</div>
                        <div className="text-xl font-bold mt-1">{topFeatureContributor?.name || "N/A"}</div>
                        <div className="text-sm mt-1">{topFeatureContributor?.featurePoints || 0} feature points</div>
                        <div className="text-xs text-muted-foreground mt-1">
