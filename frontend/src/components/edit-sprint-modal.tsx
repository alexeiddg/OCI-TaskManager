...
export function SprintEditModal({ open, onOpenChange, sprint, projects, onSave }: SprintEditModalProps) {
    ...
    useEffect(() => {
        if (sprint) {
            setFormData({
                ...
                endDate: sprint.endDate ? new Date(sprint.endDate) : new Date(Date.now() + 14 * 24 * 60 * 60 * 1000),
            })
        } else {
            ...
            <DialogContent className="sm:max-w-[600px]">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {sprint ? "Update the details of your sprint." : "Fill in the details to create a new sprint."}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                                                    ...
                                                    {projects.map((project) => (
                                                        ...
                                                        >
                                                            <Check
                                                                className={cn(
                                                                    "mr-2 h-4 w-4",
                                                                    formData.project?.id === project.id ? "opacity-100" : "opacity-0",
                                                                )}
                                                            />
                                                            {project.name}
                                                        </CommandItem>
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <Select value={formData.status} onValueChange={handleStatusChange}>
                                ...
                                <SelectContent>
                                    <SelectItem value={SprintStatus.PLANNING}>Planning</SelectItem>
                                    <SelectItem value={SprintStatus.ACTIVE}>Active</SelectItem>
                                    <SelectItem value={SprintStatus.COMPLETED}>Completed</SelectItem>
                                    <SelectItem value={SprintStatus.CANCELLED}>Cancelled</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <div className="flex items-center space-x-2 col-span-3">
                                ...
                                <Label htmlFor="isActive" className="text-sm text-muted-foreground">
                                    {formData.isActive ? "Sprint is active" : "Sprint is inactive"}
                                </Label>
                            </div>
                        </div>
                    ...
                    {sprint.createdAt ? format(new Date(sprint.createdAt), "PPP 'at' p") : "N/A"}
                  </span>
                                </div>
                      ...
                      {format(new Date(sprint.updatedAt), "PPP 'at' p")}
                                ...
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label className="text-right text-muted-foreground">Tasks</Label>
                                    <span className="col-span-3 text-sm text-muted-foreground">
