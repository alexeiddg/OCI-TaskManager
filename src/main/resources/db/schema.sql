...
create table sprints (is_active number(1,0) check (is_active in (0,1)), status number(3,0) check (status between 0 and 3), created_at timestamp(6) not null, end_date timestamp(6) not null, id number(19,0) generated by default as identity, project_id number(19,0) not null, start_date timestamp(6) not null, updated_at timestamp(6), sprint_name varchar2(255 char) not null, sprint_description clob, primary key (id));
create table task_audit (audit_id number(19,0) generated by default as identity, changed_at timestamp(6) not null, changed_by number(19,0) not null, task_id number(19,0) not null, change_type varchar2(20 char) not null check (change_type in ('CREATE','UPDATE','DELETE','STATUS_CHANGE','ASSIGNMENT_CHANGE')), new_status varchar2(20 char) check (new_status in ('TODO','IN_PROGRESS','DONE')), old_status varchar2(20 char) check (old_status in ('TODO','IN_PROGRESS','DONE')), change_comment clob, primary key (audit_id));
